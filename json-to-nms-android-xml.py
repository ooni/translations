...
def dict_to_android_xml(d, out_path, base_d):
    with open(out_path, 'w') as out_file:
        out_file.write('<?xml version="1.0" encoding="utf-8"?>\n')
        out_file.write('<resources>\n')
        for key, text in base_d.items():
            if key in d:
                text = d[key]
            key = key.replace('.', '_')
...
def parse_args():
    ...
    p.add_argument('--base', metavar='PATH', help='ooni base xml input path', required=True)
    opt = p.parse_args()
    return opt
...
def main():
    opt = parse_args()
    dict_to_android_xml(load_json(opt.json), opt.xml, load_json(opt.base))
